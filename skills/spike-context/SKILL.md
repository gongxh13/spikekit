...
description: >-
  ...
  AGENTS.md is out of date, fix it", "bootstrap project context", or "/spike-context". The
  difference from Claude Code's built-in `/init`: `/init` analyzes the codebase and
  writes down the commands it *infers*; `/spike-context` runs those commands first and only
  writes the ones that actually worked.
---

# spike-context: agent project context, with the commands actually run
...
`/spike-context`'s whole reason to exist: **run the commands you're about to recommend, keep
...
This is the step that makes `/spike-context` worth using. **It always runs — no
...
- **Top-of-file provenance HTML comment** — block HTML comments are stripped from
  ...
  ```
  <!-- Generated by spike-context on <date>. The commands under "Environment setup",
       "Running tests", and "Lint & format" / "Type checking" were executed and verified
       on this date. Re-run /spike-context to refresh. -->
  ```

  In improve mode, say "Updated by spike-context on <date>" and note that corrections from
