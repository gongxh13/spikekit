...
description: >-
  ...
  AGENTS.md is out of date, fix it", "bootstrap project context", or "/spike-runbook". The
  difference from Claude Code's built-in `/init`: `/init` analyzes the codebase and
  writes down the commands it *infers*; `/spike-runbook` runs those commands first and only
  writes the ones that actually worked.
---

# spike-runbook: agent project context, with the commands actually run
...
`/spike-runbook`'s whole reason to exist: **run the commands you're about to recommend, keep
...
This is the step that makes `/spike-runbook` worth using. **It always runs — no
...
- **Top-of-file provenance HTML comment** — block HTML comments are stripped from
  ...
  ```
  <!-- Generated by spike-runbook on <date>. The commands under "Environment setup",
       "Running tests", and "Lint & format" / "Type checking" were executed and verified
       on this date. Re-run /spike-runbook to refresh. -->
  ```

  In improve mode, say "Updated by spike-runbook on <date>" and note that corrections from
